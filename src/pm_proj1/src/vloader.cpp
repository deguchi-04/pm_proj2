...
int main(int argc, char **argv)
{
    ...
    while (ros::ok)
    {
        ...
        while (cap.isOpened())
        {
            ...
            if(tracked.x >= 345 && path == "../project_pm/src/pm_proj1/src/videoTennis.mp4"){
                cv::putText(frame, "COLLISION", cv::Point(450, 30), cv::FONT_HERSHEY_DUPLEX, 1, color3);
            }
            if(tracked.x >= 170 && tracked.y >=553 && path == "../project_pm/src/pm_proj1/src/videoPlastic.mp4"){
                cv::putText(frame, "COLLISION", cv::Point(900, 30), cv::FONT_HERSHEY_DUPLEX, 1, color3);
            }
            center.x = com.x;
            center.y = com.y;
            center.z = 0;
